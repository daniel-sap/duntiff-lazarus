...
- `Source/`
  ...
  - `DunTif.TiffParser.pas` — TIFF IFD parsing for Milestone 1 (`TDunTifTiffParser.ParseSingleFrame`)
  - `DunTif.DecodeBaseline.pas` — Milestone 1 baseline decode into `TFPMemoryImage`
- `Package/`
...
- `Doc/`
  ...
  - `TIFF_NOTES.md` / `TIFF_NOTES.bg.md` — TIFF tags/defaults + Milestone 1 limitations
...
## Supported TIFF subset today (Milestone 1)

The pure Pascal path supports **baseline uncompressed strip TIFF**:
...
1. Milestone 1 (current): baseline uncompressed RGB/Gray + strips (pure Pascal read path)
