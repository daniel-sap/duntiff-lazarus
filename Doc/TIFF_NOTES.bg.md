# Бележки за TIFF спрямо DunTif (Milestones 1–2)
...
Milestones 1–2 са насочени към **strip изображения** с **некомпресирани** или **PackBits** данни:
...
При **`Compression = 32773`** всяка стойност в `StripByteCounts` е **компресираният** размер на strip-а; след PackBits декомпресия резултатът трябва да е `редовеВStrip * ширина * байтовеНаПиксел`.
...
## Тагове, които DunTif чете (минимум за Milestones 1–2)
...
| Compression | 259 | SHORT; `1` или `32773` |
| PhotometricInterpretation | 262 | SHORT; за Milestones 1–2 трябва да е `0`, `1` или `2` |
...
За **PackBits** (`32773`) `StripByteCounts` е **компресираният** размер; след декомпресия трябва да се получи точно този суров размер.
...
- `EDunTifParseError` — невалидна TIFF структура, неподдържан тип на таг за Milestones 1–2, четене извън потока, несъответствие на strip размери.
